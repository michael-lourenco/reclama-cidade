  ...
  useEffect(() => {
    ...
    const links: SankeyLink[] = [
      // Reportado -> Em Análise
      {
        source: 0,
        target: 1,
        value: markers.filter((m) => m.currentStatus === ProblemStatus.UNDER_ANALYSIS).length || 5,
      },
      // Em Análise -> Verificado
      {
        source: 1,
        target: 2,
        value: markers.filter((m) => m.currentStatus === ProblemStatus.VERIFIED).length || 4,
      },
      // Verificado -> Em Andamento
      {
        source: 2,
        target: 3,
        value: markers.filter((m) => m.currentStatus === ProblemStatus.IN_PROGRESS).length || 3,
      },
      // Em Andamento -> Resolvido
      {
        source: 3,
        target: 4,
        value: markers.filter((m) => m.currentStatus === ProblemStatus.RESOLVED).length || 2,
      },
      // Resolvido -> Fechado
      {
        source: 4,
        target: 5,
        value: markers.filter((m) => m.currentStatus === ProblemStatus.CLOSED).length || 1,
      },
      // Fechado -> Reaberto
      {
        source: 5,
        target: 6,
        value: markers.filter((m) => m.currentStatus === ProblemStatus.REOPENED).length || 1,
      },
      // Reaberto -> Em Análise
      {
        source: 6,
